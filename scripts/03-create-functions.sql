...
-- Function to update trending status based on engagement
CREATE OR REPLACE FUNCTION update_trending_articles()
RETURNS void AS $$
BEGIN
  -- Reset all trending flags
  UPDATE public.articles SET is_trending = false;
  
  -- Set trending for top articles in last 24 hours
  UPDATE public.articles 
  SET is_trending = true
  WHERE id IN (
    SELECT id FROM get_trending_articles(10)
  );
END;
$$ LANGUAGE plpgsql;
